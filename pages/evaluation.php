...
if(isset($_POST["save"]))
{
 $query=mysqli_query($con,"select * from plan where id='$id'");
    $row=mysqli_fetch_array($query); 
    $result=$_POST["result"];
    if($result>100)
    {
        array_push($error,"the percentage must be  hundred(100)");
    }
    else if($row["status"]==1)
{
array_push($error,"Plans are still pending");
}
else if($row["status"]==3)
{
array_push($error,"Your plans are rejected");
}
else
    {
    $query=mysqli_query($con,"update plan set result='$result' where id='$id'");
    array_push($error,"Evaluation successfully made");
    }
}
...
<?php include '../error.php'?>
            <h3>Evaluation in percentage(%)</h3>
...
<input type="text" name="result" class="form-control" value="<?php echo $row["result"];?>" required="required" autocomplete="off">
</div>
<button type="submit" name="save" class="btn btn-o btn-primary">Save</button>
</form>
<?php 
}
?>
